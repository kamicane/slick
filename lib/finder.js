...
var combinators = {
    ...
    "+": function(node, part, push){ // next sibling
        while ((node = node.nextSibling)) if (node.nodeType == 1){
            if (this.match(node, part)) this.push(node)
            break
        }
    },
    ...
    "^": function(node, part, push){ // first child
        ...
        if (node){
            if (node.nodeType == 1) if (this.match(node, part)) push(node)
            else combinators['+'].call(this, node, part, push)
        }
    },
    ...
    "!^": function(node, part, push){ // last child
        ...
        if (node){
            if (node.nodeType == 1) if (this.match(node, part)) push(node)
            else combinators['+'].call(this, node, part)
        }
    },
