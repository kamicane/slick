...
var HAS = {
    ...
    QUERY_SELECTOR: function(test){
        ...
        var res = test.querySelectorAll('.MiX').length === 1
        return res
    },
...
Finder.prototype.search = function(context, expression, found, one){
    ...
    for (var i = 0, expression; expression = expressions[i++];){
        ...
        for (var n = 0, node; node = nodes[n++];) if (!nextExpression || this.validate(node, expressionIndex, expression)){
...
Finder.prototype.validate = function(node, expressionIndex, expression){
    ...
    var check = function(node){
        return (self.match(node, bit) && (!nextExpression || self.validate(node, expressionIndex, expression)))
    }
