...
Finder.prototype.search = function(context, expression, found){
    ...
    main: for (var i = 0; expression = expressions[i++];){
        ...
        nodes = this.last(context, expression[expressionLength - 1], uniques)
...
Finder.prototype.validate = function(context, node, expressionIndex, expression){
    ...
    switch(combinator){
        ...
        // children of
        case " ": while ((node = node.parentNode)){
        ...
        // previous sibling of
        case "!+": while ((node = node.nextSibling)){
            if (node.nodeType === 1) return check(node)
        } break
...
var createNTHPseudo = function(child, sibling, positions, ofType){
        ...
        if (!this[positions][uid]){
            ...
            if (ofType){
                ...
                do {
                    ...
                    this[positions][uniqueID(el)] = count++
                } while ((el = el[sibling]))
            } else {
                do {
...
var pseudos = {
    ...
    'empty': function(node){
        ...
        return !(node && node.nodeType === 1) && !(node.innerText || node.textContent || '').length
    },

    'not': function(node, expression){
        return !slick.match(node, expression)
    },
    ...
    'nth-child': createNTHPseudo('firstChild', 'nextSibling', 'posNTH'),

    'nth-last-child': createNTHPseudo('lastChild', 'previousSibling', 'posNTHLast'),

    'nth-of-type': createNTHPseudo('firstChild', 'nextSibling', 'posNTHType', true),

    'nth-last-of-type': createNTHPseudo('lastChild', 'previousSibling', 'posNTHTypeLast', true),

    'index': function(node, index){
        return pseudos['nth-child'].call(this, node, '' + (+index + 1))
    },

    'even': function(node){
        return pseudos['nth-child'].call(this, node, '2n')
    },

    'odd': function(node){
        return pseudos['nth-child'].call(this, node, '2n+1')
    },
    ...
    'focus': function(node){
        ...
        return doc.activeElement === node && (node.href || node.type || slick.hasAttribute(node, 'tabindex'))
    },
...
Finder.prototype.match = function(node, bit, noTag, noId, noClass){
    ...
    if (bit.pseudos) for (i = 0; part = bit.pseudos[i++];){
        ...
        if (pseudos[name]) return pseudos[name].call(this, node, value)
